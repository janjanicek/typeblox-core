...
class Blox extends EventEmitter_1.EventEmitter {
  constructor({
    onUpdate,
    id,
    type,
    ...
    this.updateContent = () => {
      ...
      this.content =
        (_b =
          (_a = this.getContentElement()) === null || _a === void 0
            ? void 0
            : _a.innerHTML) !== null && _b !== void 0
          ? _b
          : "";
    };
    this.getContent = () => {
      this.updateContent();
      return `<${constants_1.BLOCKS_SETTINGS[this.type].tag}>${this.content}</${constants_1.BLOCKS_SETTINGS[this.type].tag}>`;
    };
    this.setContent = (contentString) => {
      const parser = new DOMParser();
      const doc = parser.parseFromString(contentString, "text/html");
      const wrapperTag = constants_1.BLOCKS_SETTINGS[this.type].tag;
      ...
      this.emit(constants_1.EVENTS.blocksChanged);
    };
  ...
  afterToggle() {
    ...
    this.emit(constants_1.EVENTS.styleChange);
  }
  toggleBold() {
